        ...
        for (PossibleCharacteristics.CharacteristicField field : characteristicsFields) {
            ...
            switch (type) {
                ...
                case NAME_SET:
                    ...
                    String select = (String) ((ChoiceBox<String>) field.control).getSelectionModel().getSelectedItem();
                    if (select == null) {
                        WindowBuilder.alert(Alert.AlertType.WARNING, "Cannot save instance", "Item is not selected in " + field.name);
                        return;
                    }
                    map.put(field.name, new Characteristic(select));
                    break;
                case NUMBER:
                    int number;
                    try {
                        number = Integer.parseInt(((TextField) field.control).getText());
                ...
                case NUMBER_RANGE:
                    try {
                        int from = Integer.parseInt(((TextField) field.control).getText());
                        int to = from;
